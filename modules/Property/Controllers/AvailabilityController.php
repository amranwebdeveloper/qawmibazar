...
use Modules\Property\Models\PropertyDokanBooking;
...
class AvailabilityController extends FrontendController{
    ...
    public function index(Request $request){
        ...
        $page_title = __('Dokan Availability');

        return view($this->indexView,compact('rows','breadcrumbs','current_month','page_title','request'));
    }

    public function loadDates(Request $request,$property_id){
        ...
        if(!$this->hasPropertyPermission($property_id))
        {
         ...
         /**
        * @var $dokan PropertyDokan
        */
        $dokan = $this->dokanClass::find($request->query('id'));
        if(empty($dokan)){
            return $this->sendError(__('Property not found'));
        }
        ...
        $bookings = $dokan::getBookingInRanges($request->query('start'),$request->query('end'));
        if(!empty($bookings))
        {
    ...
    public function store(Request $request){
        ...
        $dokan = $this->dokanClass::find($request->input('target_id'));
        $target_id = $request->input('target_id');

        if(empty($dokan)){
            return $this->sendError(__('Dokan not found'));
        }

        if(!$this->hasPermission('property_manage_others')){
            if($this->currentproperty->create_user != Auth::id()){
        ...
        {
            $date = $this->dokanDateClass::where('start_date',date('Y-m-d',$i))->where('target_id',$target_id)->first();

            if(empty($date)){
                $date = new $this->dokanDateClass();

    ...
    <div class="booking-review-content">
        <div class="review-section">
            <div class="service-info">
                ...
                @if($vendor->hasPermissionTo('dashboard_agent_access') and !$vendor->hasPermissionTo('dashboard_access'))
                    <div class="mt-2">
                        <i class="icofont-info-circle"></i>
                        {{ __("Vendor") }}: <a href="{{route('user.profile',['id'=>$vendor->id])}}" target="_self" >{{$vendor->getDisplayName()}}</a>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="review-section total-review">
            <ul class="review-list">
				@php $dokans = \Modules\Property\Models\PropertyDokanBooking::getByBookingId($booking->id) @endphp
				@if(!empty($dokans))
					@foreach($dokans as $dokan)
						<li class="flex-wrap">
							<div class="label">{{$dokan->dokan->title}} * {{$dokan->number}}</div>
							<div class="val">
								{{format_money($dokan->price * $dokan->number)}}
							</div>
						</li>
					@endforeach
						<li class="flex-wrap">
                        ...
                        <ul>
                            ...
                            <li>
                                <div class="label">
                                    ...
                                    {{--<small>
                                        ({{ $type['type'] == "per_day" ? "/".__("day") : "" }}/{{ __("dokan") }})
                                    </small>--}}
                                </div>
                                <div class="val">
				...
				@if(!empty($dokans))
					...
					@foreach($dokans as $dokan)
						<li class="mb-3 pb-1 border-bottom">
							<h6 class="label text-center font-weight-bold mb-1">{{$dokan->dokan->title}} * {{$dokan->number}}</h6>
							@if(!empty($dateDetail[$dokan->dokan_id]))
								<div>
									@includeIf("Property::frontend.booking.detail-dokan",['dokanDate'=>$dateDetail[$dokan->dokan_id]])
								</div>
							@endif
							<div class="d-flex justify-content-between font-weight-bold px-2">
								<span>{{__("Total:")}}</span>
								<span>{{format_money($dokan->price * $dokan->number)}}</span>
							</div>
						</li>
					@endforeach
					</ul>

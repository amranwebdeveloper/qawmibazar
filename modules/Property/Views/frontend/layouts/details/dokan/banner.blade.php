<div class="home10-mainslider">
    <?php $banner_images = $dokanrow->getBannerImagesDokan(); ?>
    @if ($banner_images || $dokanrow->banner_image_id)
        <div class="container-fluid p0 dokan">
            <div class="row">
                <div class="col-lg-12">
                    <div class="main-banner-wrapper home10">
                        <div class="dokan-banner">
                            <img src="{{ get_file_url($dokanrow->banner_image_id, 'large') }}" alt="{{ $dokanrow->title }}" />
                        </div>
                ...
                <div class="single_property_title listing_single_v1">
                    <div class="media">
                        <div class="media-body">
                            <h1 class="mt-0">{{ $dokanrow->title }} - {{ $row->title }} @if (!empty($dokanrow->location->name))
                                    in {{ $dokanrow->location->name }}
                                @else
                                @endif
                            </h1>
                        </div><!-- /.main-banner-wrapper -->
                        ...
                        <div class="media-body ">
                            ...
                            <div class="sspd_review listing_single_v1">
                                <ul class="mb0">
                                    ...
                                    @endif
                                    @if ($dokanrow->price_range)
                                        <li class="list-inline-item ml20">
                                            <span class="price_range">
                                                @for ($i = 0; $i < $dokanrow->price_range; $i++)
                                                    $
                                                @endfor
                                            </span>
                                        </li>
                                    ...
                                    @if ($dokanrow->view > 0)
                                        <li class="list-inline-item ml20">
                                            <span class="price_range">
                                                <i class="icofont-eye-alt"></i> {{ $dokanrow->view }}
                                            </span>
                                        </li>
                                    @endif
                                </ul>
                    ...
                    <div class="spss style2 listing_single_v1 mt30 float-left fn-lg">
                        <ul class="mb0">
                            ...
                            <li class="list-inline-item icon">
                                <a href="#" class="service-wishlist {{ $dokanrow->isWishListDokan() }}"
                                   data-id="{{ $dokanrow->id }}" data-type="{{ $dokanrow->type }}">
                                    <span class="fa fa-heart-o"></span>
                                </a>
                            </li>
                            <li class="list-inline-item"><a class="text-white" href="#">{{ $dokanrow->isWishListDokan() ? __('Saved') : __('Save') }}</a></li>
                        </ul>
                    </div>

...
if ($name == 'category_id') {
    ...
    $placeHolder = __('Ex: Search Business, Category etc...');
}
?>
            ...
            <select name="{{ $name }}" class=" form-control js-searchBox">
                ...
                @if ($name != 'location_id' && $name != 'category_id')
                    @foreach ($list as $item)
                        <option value="{{ $item->id }}" @if (Request::input('property_type') == $item->id && $name == 'property_type') selected @endif
                            ...
                            @if (Request::input('year_built') == $item->id && $name == 'year_built') selected @endif>{{ $item->name }}</option>
                    @endforeach
                @elseif($name == 'location_id')
                    @foreach ($list_json_location as $item)
                        <option value="{{ $item['id'] }}" @if (Request::input('location_id') == $item['id'] && $name == 'location_id') selected @endif>
                            {{ $item['title'] }}</option>
                    @endforeach
                @else

    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <div class="control-action">
                ...
                <a href="{{ url('/').'/listing/'.$property->slug.'/'.$dokan->slug . '/' . $row->slug }}" class="btn btn-success">{{ __('View') }}</a>
                @if (Auth::user()->hasPermissionTo('property_update'))
                    <a href="{{ route('property.vendor.product.edit', ['dokan_id' => $dokan->id, 'id' => $row->id]) }}"
                        class="btn btn-warning">{{ __('Edit') }}</a>
                @endif
{{--                @if (Auth::user()->hasPermissionTo('property_update'))--}}
{{--                    <a href="{{ route('property.vendor.product.delete', ['dokan_id' => $dokan->id, 'id' => $row->id]) }}"--}}
{{--                        class="btn btn-danger" data-confirm="<?php echo e(__('Do you want to delete?')); ?>">{{ __('Del') }}</a>--}}
{{--                @endif--}}
                @if ($row->status == 'publish')
                    <a href="{{ route('property.vendor.product.bulk_edit', ['dokan_id' => $dokan->id, 'id' => $row->id, 'action' => 'make-hide']) }}"
                        class="btn btn-secondary">{{ __('Make hide') }}</a>
                @endif
                @if ($row->status == 'draft')
                    <a href="{{ route('property.vendor.product.bulk_edit', ['dokan_id' => $dokan->id, 'id' => $row->id, 'action' => 'make-publish']) }}"

            ...
            <div class="breadcrumb_content">
                <div class="row">
                    ...
                    <div class="col-6 text-right">
                        <a href="{{ route('property.vendor.contact') }}"
                            class="btn btn-primary add-new-property">{{ __('View Contact') }}</a>
                        <a href="{{ route('property.vendor.create') }}"
                            class="btn btn-primary add-new-property">{{ __('Add new Business') }}</a>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div class="candidate_revew_search_box course fn-520">
                                <form class="form-inline my-2" method="get" action="{{ route('property.vendor.index') }}">
                                    <input class="form-control mr-sm-2" type="search" name="s"
                                        placeholder="{{ __('Search Properties') }}" aria-label="Search"
                        ...
                        <ul class="mb0 mt10">
                            ...
                            <li class="list-inline-item">
                                <select class="selectpicker show-tick" onchange="this.form.submit()" name="status">
                                    ...
                                    <option value="publish" @if (request()->input('status') == 'publish') selected @endif>
                                        {{ __('Publish') }}</option>
                                    <option value="draft" @if (request()->input('status') == 'draft') selected @endif>
                                        {{ __('Draft') }}</option>
                                </select>
                            </li>
                        </ul>
                    ...
                    <table class="table">
                        <thead>
                            <tr class="carttable_row">
                                <th class="cartm_title">{{ __('Name (View)') }}</th>
                                <th class="dn-lg"></th>

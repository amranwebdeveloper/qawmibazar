<tr>
    ...
    <td>
        @if($service = $booking->service)
            <a target="_self" href="{{$service->getDetailUrl()}}">
                {{$service->title}}
            </a>
        @else
            {{__("[Deleted]")}}
    ...
    <td class="a-hidden">
        ...
        @if(!empty($dokans))
            @foreach($dokans as $dokan)
                    <div class="label">{{$dokan->dokan->title}} * {{$dokan->number}} = {{format_money_main($dokan->price * $dokan->number)}} </div>
            @endforeach
        @endif
    </td>
    <td>
    ...
    <td width="2%">
        ...
        @if(!empty(setting_item("property_allow_vendor_can_change_their_booking_status")))
            ...
            <div class="dropdown-menu">
                @if(!empty($statues))
                    @foreach($statues as $status)
                        <a href="{{ route("property.vendor.booking_report.bulk_edit" , ['id'=>$booking->id , 'status'=>$status]) }}">
        ...
        @endif
        @if(!empty(setting_item("property_allow_vendor_can_change_paid_amount")))

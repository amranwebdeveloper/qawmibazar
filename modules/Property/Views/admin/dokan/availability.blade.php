...
@section('content')
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{ __('Store Availability Calendar') }}</h1>
        </div>
        ...
        <div class="panel">
            <div class="panel-body">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-right">
                        @if ($rows->total() > 0)
                            <span  class="count-string">{{ __('Showing :from - :to of :total dokans', ['from' => $rows->firstItem(), 'to' => $rows->lastItem(), 'total' => $rows->total()]) }}</span>
                        @endif
                    </div>
                </div>
            </div>
        ...
        @else
            <div class="alert alert-warning">{{ __('No dokans found') }}</div>
        @endif
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="row form_modal_calendar form-horizontal" novalidate onsubmit="return false">
                        ...
                        <div class="col-md-6" v-show="form.active">
                            <div class="form-group">
                                <label>{{ __('Number of dokan') }}</label>
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab', function(e) {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: {
                    url: "{{ route('property.admin.dokan.availability.loadDates', ['property_id' => $property->id]) }}",
                    extraParams: {
                        id: lastId,
                    }
                },
        ...
        formModal = new Vue({
            ...
            methods: {
                ...
                saveForm: function() {
                    ...
                    $.ajax({
                        url: '{{ route('property.admin.dokan.availability.store', ['property_id' => $property->id]) }}',
                        data: this.form,
                        dataType: 'json',
                        method: 'post',

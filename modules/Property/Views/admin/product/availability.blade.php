            ...
            <div class="panel-body">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-right">
                        @if ($rows->total() > 0)
                            <span
                                class="count-string">{{ __('Showing :from - :to of :total dokans', ['from' => $rows->firstItem(), 'to' => $rows->lastItem(), 'total' => $rows->total()]) }}</span>
                        @endif
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab', function(e) {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: {
                    url: "{{ route('property.admin.product.availability.loadDates', ['dokan_id' => $dokan->id]) }}",
                    extraParams: {
                        id: lastId,
                    }
                },
        ...
        formModal = new Vue({
            ...
            methods: {
                ...
                saveForm: function() {
                    ...
                    $.ajax({
                        url: '{{ route('property.admin.product.availability.store', ['dokan_id' => $dokan->id]) }}',
                        data: this.form,
                        dataType: 'json',
                        method: 'post',

...
class PropertyDokan extends Bookable
{
    use SoftDeletes;
    protected $table = 'bc_property_dokans';
    public $type = 'dokan';
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        return url(app_get_locale(false, false, '/') . config('property.property_route_prefix') . '/' . $this->slug);
    }
    public function saveCloneByID($clone_id)
    {
        ...
        foreach ($selected_terms as $term_id) {
            $this->propertyDokanTermClass::firstOrCreate([
                'term_id' => $term_id,
                'target_id' => $new->id
            ]);
        }
        //Language
        $langs = $this->propertyDokanTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {

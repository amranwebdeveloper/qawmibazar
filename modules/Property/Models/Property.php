...
use Modules\User\Models\UserWishListDokan;
...
class Property extends Bookable
{
    ...
    protected $userWishListDokanClass;
    protected $termClass;
    protected $attributeClass;
    protected $dokanClass;
    protected $tmp_dokans = [];
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->userWishListDokanClass = UserWishListDokan::class;
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        if ($include_param) {
            ...
            if (!empty($dokan = request()->input('dokan'))) {
                $param['dokan'] = $dokan;
            }
        }
    ...
    public function addToCart(Request $request)
    {
        ...
        $total = 0;
        $total_dokan_selected = 0;
        if (!empty($this->tmp_selected_dokans)) {
            foreach ($this->tmp_selected_dokans as $dokan) {
                if (isset($this->tmp_dokans_by_id[$dokan['id']])) {
                    $total += $this->tmp_dokans_by_id[$dokan['id']]->tmp_price * $dokan['number_selected'];
                    $total_dokan_selected += $dokan['number_selected'];
                }
            }
        }
        ...
        if ($check) {
            ...
            if (!empty($this->tmp_selected_dokans)) {
                foreach ($this->tmp_selected_dokans as $dokan) {
                    if (isset($this->tmp_dokans_by_id[$dokan['id']])) {
                        $propertyDokanBooking = new PropertyDokanBooking();
                        $propertyDokanBooking->fillByAttr([
                            'dokan_id',
                            'parent_id',
                            'start_date',
                            'end_date',
                            ...
                            'number'     => $dokan['number_selected'],
                            'booking_id' => $booking->id,
                            'price'      => $this->tmp_dokans_by_id[$dokan['id']]->tmp_price
                        ]);
                        $propertyDokanBooking->save();
                    }
                }
            }
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        $total_dokans = array_sum(array_column($request->input('dokans', 'number_selected'), "number_selected"));
        $selected_dokans = Arr::where($request->input('dokans'), function ($value, $key) {
            return !empty($value['number_selected']) and $value['number_selected'] > 0;
        });
        if ($total_dokans <= 0 or empty($selected_dokans)) {
            return $this->sendError(__("Please select at lease one dokan"));
        }
        ...
        $dokans = $this->getDokansAvailability(request()->input());
        $dokans_by_id = [];
        if (empty($dokans))
            return $this->sendError(__("There is no dokan available at your selected dates"));
        foreach ($this->tmp_dokans as $dokan) {
            $dokans_by_id[$dokan['id']] = $dokan;
        }
        $dokans_ids = array_column($dokans, 'id');
        $numberDays = (abs(strtotime($end_date) - strtotime($start_date)) / 86400) + 1;

        $total_adult_select = 0;
        $total_child_select = 0;
        foreach ($selected_dokans as $dokan) {
            if (!in_array($dokan['id'], $dokans_ids) or $dokan['number_selected'] > $dokans_by_id[$dokan['id']]->tmp_number) {
                return $this->sendError(__("Your selected dokan is not available. Please search again"));
            }
            if (!empty($dokans_by_id[$dokan['id']]->min_day_stays) and  $numberDays < $dokans_by_id[$dokan['id']]->min_day_stays) {
                return $this->sendError(__("The :name need to select at least :number days", ['name' => $dokans_by_id[$dokan['id']]->title, 'number' => $dokans_by_id[$dokan['id']]->min_day_stays]));
            }
            if (!empty($dokans_by_id[$dokan['id']])) {
                $total_adult_select += $dokans_by_id[$dokan['id']]->adults * $dokan['number_selected'];
                $total_child_select += $dokans_by_id[$dokan['id']]->children * $dokan['number_selected'];
            }
        }
        if (!empty($adults = $request->input('adults')) and $adults > $total_adult_select) {
            return $this->sendError(__("Sorry, the current dokans are not enough for adults"));
        }
        if (!empty($adults = $request->input('children')) and $adults > $total_child_select) {
            return $this->sendError(__("Sorry, the current dokans are not enough for children"));
        }
        $this->tmp_dokans_by_id = $dokans_by_id;
        $this->tmp_selected_dokans = $selected_dokans;
        return true;
    }
    public function beforeCheckout(Request $request, $booking)
    {
        ...
        $dokansBookings = PropertyDokanBooking::select("dokan_id")->where("booking_id", $booking->id)->get();
        $dokan_ids = $dokansBookings->pluck('dokan_id')->toArray();
        $dokans = PropertyDokan::whereIn('id', $dokan_ids)->get();
        foreach ($dokans as $dokan) {
            if (!$dokan->isAvailableAt($filters)) {
                return $this->sendError(__("There is no dokan available at your selected dates"));
            }
        }
    }
    ...
    public function getBookingData()
    {
        ...
        if (!empty($children = request()->input('dokan'))) {
            $booking_data['dokan'] = $children;
        }
        $lang = app()->getLocale();
    ...
    public function getPropertyNumberInLocation($location_id)
    {
        $number = 0;
        if (!empty($location_id)) {
            $number = parent::join('bc_locations', function ($join) use ($location_id) {
                $join->on('bc_locations.id', '=', $this->table . '.location_id')->orWhere('bc_locations.parent_id', '=', $this->table . '.location_id');
            })->where($this->table . ".status", "publish")->with(['translations'])->count($this->table . ".id");
        }
        if (empty($number)) return false;
        if ($number > 1) {
    ...
    {
        return $this->hasOne($this->UserWishListDokanClass, 'object_id', 'id')->where('object_model', $this->type)->where('user_id', Auth::id() ?? 0);
    }
    ...
    {
        return $this->hasMany($this->dokanClass, 'parent_id')->where('status', "publish")->with("translations");
    }

    public function getDokansAvailability($filters = [])
    {
        $dokans = $this->dokans;
        $res = [];
        $this->tmp_dokans = [];
        foreach ($dokans as $dokan) {
            if ($dokan->isAvailableAt($filters)) {
                $translation = $dokan->translateOrOrigin(app()->getLocale());
                $terms = Terms::getTermsByIdForAPI($dokan->terms->pluck('term_id'));
                $term_features = [];
                $i = 0;
                if (!empty($terms)) {
                ...
                $res[] = [
                    ...
                    'price'           => $dokan->tmp_price ?? 0,
                    'size_html'       => $dokan->size ? size_unit_format($dokan->size) : '',
                    'beds_html'       => $dokan->beds ? 'x' . $dokan->beds : '',
                    'adults_html'     => $dokan->adults ? 'x' . $dokan->adults : '',
                    'children_html'   => $dokan->children ? 'x' . $dokan->children : '',
                    'number_selected' => 0,
                    'number'          => (int)$dokan->tmp_number ?? 0,
                    'min_day_stays'   => $dokan->min_day_stays ?? 0,
                    'image'           => $dokan->image_id ? get_file_url($dokan->image_id, 'medium') : '',
                    'tmp_number'      => $dokan->tmp_number,
                    'gallery'         => $dokan->getGallery(),
                    'price_html'      => format_money($dokan->tmp_price) . '<span class="unit">/' . ($dokan->tmp_nights ? __(':count nights', ['count' => $dokan->tmp_nights]) : __(":count night", ['count' => $dokan->tmp_nights])) . '</span>',
                    'price_text'      => format_money($dokan->tmp_price) . '/' . ($dokan->tmp_nights ? __(':count nights', ['count' => $dokan->tmp_nights]) : __(":count night", ['count' => $dokan->tmp_nights])),
                    'terms'           => $terms,
                    'term_features'   => $term_features
                ];
                $this->tmp_dokans[] = $dokan;
            }
        }
        return $res;
    }
    ...
    {
        $dokans = \Modules\Property\Models\PropertyDokanBooking::getByBookingId($booking->id);
        $dokansIds = $dokans->pluck('dokan_id')->toArray();
        $dokansNumber = $dokans->pluck('number', 'dokan_id')->toArray();
        $startDate = $booking->start_date;
        $endDate = $booking->end_date;
        $query = PropertyDokanDate::query();
        $query->whereIn('target_id', $dokansIds);
        $query->where('start_date', '>=', date('Y-m-d H:i:s', strtotime($startDate)));
        $query->where('end_date', '<', date('Y-m-d H:i:s', strtotime($endDate)));
        $dokansDates = $query->get();
        $allDates = [];
        foreach ($dokans as $r => $dokan) {
            $period = periodDate($startDate, $endDate, false);
            foreach ($period as $dt) {
                $price = $dokan->dokan->price * $dokan->number;
                $date['price'] = $price;
                ...
                $allDates[$dokan->dokan_id][$dt->format('Y-m-d')] = $date;
            }
        }

        if (!empty($dokansDates)) {
            foreach ($dokansDates as $dokansDate) {
                $startDate = strtotime($dokansDate->start_date);
                if ($allDates[$dokansDate->target_id][date('Y-m-d', $startDate)]) {
                    $price = $dokansDate->price * $dokansNumber[$dokansDate->target_id] ?? 1;
                    $date['price'] = $price;
                    $date['price_html'] = format_money($price);
                    $date['from'] = $startDate;
                    ...
                    $allDates[$dokansDate->target_id][date('Y-m-d', $startDate)] = $date;
                }
            }
        }
        return $allDates;
    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($bathdokan = $request->query("bathdokan")) && $bathdokan > 0) {
            $model_property->where("bc_properties.bathdokan", $bathdokan);
        }
        if (!empty($beddokan = $request->query("beddokan")) && $beddokan > 0) {
            $model_property->where("bc_properties.bed", $beddokan);
        }
    ...
    public function getFeatureProperties()
    {
        return self::where('is_featured', self::IS_FEATURE)
            ...
            ->select('title', 'is_featured', 'image_id', 'price', 'bed', 'bathdokan', 'square')
            ->orderBy('id', 'desc')
            ->get();
    }
    ...
    public function getListPropertiesByTerm($term_id = false)
    {
        $model_property = parent::query()->select("bc_properties.*");
        $model_property->where("bc_properties.status", "publish");

        if (!empty($term_id)) {
            $model_property->join('bc_property_term as bcpterm', 'bcpterm.target_id', "bc_properties.id")->where('bcpterm.term_id', $term_id);
        }
        if (!empty($orderby = request()->query("orderby"))) {
            switch ($orderby) {
                case "new":
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "4",
                "data" => Attributes::getAllAttributesForApi("property")
            ]
        ];
    }
    static public function getFormSearch()
    {
        $search_fields = setting_item_array('property_search_fields');

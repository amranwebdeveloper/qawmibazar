            ...
            <div class="col-lg-12">
                <div class="home_content home1">
                    <div class="home-text text-center">
                        @if (!empty($title))
                            <h1 class="fz50">{{ $title }}</h1>
                        @endif
                        @if (!empty($sub_title))
                            <p class="fz18 color-white">
                                @php
                                    $properties = DB::table('bc_properties')
                                ...
                                <div class="wrapper">
                                    <div class="home_adv_srch_form ">
                                        @if (empty($hide_form_search))
                                            <div class="">
                                                <ul class="nav nav-tabs @if (!empty($hide_tab_form_search)) d-none @endif"
                                                    role="tablist">
                                                    @if (!empty($service_types))
                                                        ...
                                                        @foreach ($service_types as $service_type)
                                                            ...
                                                            <li role="bc_{{ $service_type }}">
                                                                <a href="#bc_{{ $service_type }}"
                                                                    class="@if ($number == 0) active @endif"
                                                                    aria-controls="bc_{{ $service_type }}"
                                                                    role="tab" data-toggle="tab">
                                                                    <i
                                                                        class="{{ $module->getServiceIconFeatured() }}"></i>
                                                    ...
                                                    @if (!empty($service_types))
                                                        ...
                                                        @foreach ($service_types as $service_type)
                                                            ...
                                                            <div role="tabpanel"
                                                                ...
                                                                id="bc_{{ $service_type }}">
                                                                @include(ucfirst($service_type) .
                                                                        '::frontend.layouts.search.form-search')
                                                            </div>
                                                            @php $number++; @endphp
                                                        @endforeach
                                                    @endif

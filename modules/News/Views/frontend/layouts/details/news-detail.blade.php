...
<section class="inner_page_breadcrumb style3"
    @if ($bg = setting_item('news_page_list_banner')) style="background-image: url({{ get_file_url($row->banner_image_id, 'full') }})" @endif>
    <div class="container">
                ...
                <div class="blog_single_post_heading text-center">
                    <div class="contents">
                        ...
                        <ul class="mb0">
                            ...
                            <li class="list-inline-item text-white"><span class="flaticon-date mr10"></span>
                                {{ display_date($row->updated_at) }} </li>
                        </ul>
                    </div>
                </div>
            </div>
                        ...
                        <ul class="blog_post_share text-right pt5">
                            @if (!empty(($tags = $row->getTags())) and count($tags) > 0)
                                @foreach ($tags as $tag)
                                    ...
                                    <li class="list-inline-item"><a href="{{ $tag->getDetailUrl(app()->getLocale()) }}"
                                            class="tag-item"> {{ $t->name ?? '' }} </a></li>
                                @endforeach
                            @endif
                        </ul>
                    </div>
                                    ...
                                    <div class="detls">
                                        @if (!empty($next->slug))
                                            <a href="{{ url('news/' . $next->slug) }}">
                                                <h5 class="text-thm">Next Post <span
                                                        class="fa fa-angle-right ml5"></span></h5>

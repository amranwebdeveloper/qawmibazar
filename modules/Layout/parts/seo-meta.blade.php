...
@if (!empty($seo_meta))
    ...
    @php
        ...
        if (!empty($page_title) and empty($seo_meta['is_homepage'])) {
            $page_title .= ' - ' . setting_item_with_lang('site_title', false, 'Qawmi Bazar');
        }
        if (empty($page_title)) {
            $page_title = setting_item_with_lang('site_title', false, 'Qawmi Bazar');
        }
    ...
    @php
        if (empty($seo_meta['seo_header'])) {
            echo setting_item_with_lang('schema_data');
        } else {
            echo $seo_meta['seo_header'];
        }
    @endphp
    {{-- Facebook share --}}

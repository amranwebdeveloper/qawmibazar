...
@section('content')
    <div class="container-fluid">
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if (!empty($rows))
                    <form method="post" action="{{ url('admin/module/review/bulkEdit') }}"
                        ...
                        <select name="action" class="form-control">
                            ...
                            <option value="1">{{ __(' Show Homepage ') }}</option>
                            <option value="2">{{ __(' Hide Homepage ') }}</option>
            ...
            <div class="col-left">
                <form method="post" action="{{ url('/admin/module/review/') }} "
                            ...
                            <tbody>
                                @if ($rows->total() > 0)
                                    @foreach ($rows as $row)
                                        ...
                                        <tr
                                            class="{{ $row->status }} @if ($row->is_homepage == 1) show_hompage @endif">
                                            ...
                                            <td>
                                                @if (!empty(($metaUser = $row->getUserInfo)))
                                                    <a
                                                        href="{{ url('/admin/module/review?customer_id=' . $metaUser->id) }}">{{ $metaUser->email ?? 'Email' }}</a>
                                                    <p>
                                                        <a
                                                            href="{{ url('/admin/module/review?s=' . $row->author_ip) }}">{{ $row->author_ip }}</a>
                                                    </p>
                                                @else
                                                    {{ __('[Author Deleted]') }}
                                                @endif
                                            ...
                                            <td>
                                                        ...
                                                        @foreach ($listImages as $oneImages)
                                                            ...
                                                            <div class="col-md-2 mb-2">
                                                                <div class="review_upload_item"
                                                                    style="background-image: url({{ @$imagesData['download'] }}); background-repeat: no-repeat; background-size: cover; background-position: center; height: 10vh;">
                                                                </div>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                ...
                                                @if (!empty($service) and !empty(($allReviewStats = $service->getReviewStats())))
                                                    @if (!empty(($metaReviews = $row->getReviewMeta())))
                                                        <a class="btn-show-info-review right" data-toggle="collapse"
                                                            href="#review-{{ $row->id }}">
                                                            {{ __('More info') }}
                                                                    ...
                                                                    @foreach ($metaReviews as $metaReview)
                                                                        @if (in_array($metaReview->name, $allReviewStats))
                                                                            <div class="item col-md-12 d-flex">
                                                                                <label
                                                                                    style="margin-right: 15px;">{{ $metaReview->name }}</label>
                                            ...
                                            <td>
                                                @if (!empty($service))
                                                    <a
                                                        href="{{ url('/admin/module/review?service_id=' . $service->id) . '&object_model=' . $service->type }}">
                                                        {{ $service->title }}
                                                    </a>
                                                    <p>
                                                        <a target="_blank" href="{{ $service->getDetailUrl() }}">
                                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                                            {{ __('View :name', ['name' => $service->getModelName()]) }}
                                                        </a>
                                                    </p>
                                                @else
                                                    {{ __('[Deleted]') }}
                                            ...
                                            <td>
                                                @if (!empty($service))
                                                    <a href="{{ url('/admin/module/review?service=' . $service->getModelName()) }}"
                                                        class="badge badge-dark">{{ $service->getModelName() }}</a>
                                                @else
                                                    {{ __('[Deleted]') }}
                                                @endif
                                            </td>
                                            <td>
                                                <a href="{{ url('/admin/module/review?status=' . $row->status) }}"
                                                    class="badge badge-{{ $row->status }}">{{ $row->status }}</a>
                                            </td>

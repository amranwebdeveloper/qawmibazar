@php
if (empty($reviewData)) {
    $reviewData = Modules\Review\Models\Review::getTotalViewAndRateAvg($dokanrow['id'], $review_service_type);
}
                    ...
                    @if ($val->title == $val2->name)
                        ...
                        <div class="col-lg-4">
                            <div class="review_content">
                                ...
                                <div class="review_point">@php echo round($val2->sbc_total,2); @endphp</div>
                            </div>
                        </div>
                    @break
                @endif
                            ...
                            <div class="review_upload_photo_list thumb-list mt30">
                                <ul>
                                    @foreach ($listImages as $key => $oneImages)
                                        ...
                                        @if ($key <= 3)
                                            <li class="list-inline-item review_upload_item gallery_item mb-3">
                                                <a href="{{ @$imagesData['download'] }}" class="popup-img-review">
                                                    @if ($key >= 3)
                                                        <div class="gallery_overlay style2 listing_single_v1"><span
                                                                class="icon"><span
                                                                    class="fz14">+{{ count($listImages) - 4 }}</span></span>
                                                        </div>
                                                    @endif
                                        ...
                                        @else
                                            <li class="hidden"><a class="popup-img-review"
                                                    href="{{ @$imagesData['download'] }}"></a></li>
                                        @endif
                                    @endforeach
                                </ul>
                            </div>
    ...
    <div class="wrapper">
        ...
        <form class="review_form review-form" id="review-form" action="{{ route('review.store') }}" method="post">
            ...
            <div class="form-group">
                <input type="hidden" name="review_service_id" value="{{ $dokanrow['id'] }}">

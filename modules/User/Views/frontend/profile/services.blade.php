...
$list_service = [];
?>
<div class="profile-service-tabs mb-0">
        ...
        <ul class="nav nav-tabs">
            ...
            @foreach ($types as $type => $moduleClass)
                @php
                    ...
                    /* if (!$user->hasPermissionTo($type . '_create')) {
                        continue;
                    } */
                    $services = $moduleClass
                        ::getVendorServicesQuery($user->id)
                        ->orderBy('id', 'desc')
                        ->paginate(6);
    ...
    <div class="tab-content">
        ...
        @foreach ($types as $type => $moduleClass)
            @php
                ...
                if (empty($list_service[$type])) {
                    continue;
                }
            @endphp
            @if (view()->exists(ucfirst($type) . '::frontend.profile.service') && $user->hasPermissionTo($type . '_create'))
                <div class="tab-pane fade @if (!$i) show active @endif"
                    id="type_{{ $type }}" role="tabpanel" aria-labelledby="pills-home-tab">
                    @include(ucfirst($type) . '::frontend.profile.service', [
                        'services' => $list_service[$type],
                    ])
                </div>
                @php $i++; @endphp
            @endif

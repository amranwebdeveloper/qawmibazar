...
<section class="our-contact">
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-5">
                <div class="form_grid">
                    <h3 class="title mb5">{{ setting_item_with_lang('contact_form_title') }}</h3>
                    {!! setting_item_with_lang('contact_form_paragraph') !!}
                    <div role="form" class="form_wrapper" lang="en-US" dir="ltr">
                        <form class="contact_form bc-contact-block-form" id="contact_form" name="contact_form"
                            method="post" action="{{ route('contact.store') }}" novalidate="novalidate">
                            {{ csrf_field() }}
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <input id="form_email" name="email" class="form-control required email"
                                            required="required" type="email" placeholder="{{ __('Email') }}">
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <input id="form_phone" name="phone" class="form-control required phone"
                                            required="required" type="text" placeholder="{{ __('Phone') }}">
                                    </div>
                                </div>
                                <div class="col-sm-12">
                                    <div class="form-group">
                                    ...
                                    <div class="form-group mb0">
                                        <button type="submit"
                                            class="submit btn btn-lg btn-thm">{{ __('Send Message') }} <i
        ...
        jQuery(function($) {
            @if (setting_item('map_contact_lat') && setting_item('map_contact_long'))
                new BravoMapEngine('map-canvas', {
                    disableScripts: true,
                    fitBounds: true,
                    center: [{{ setting_item('map_contact_lat') }},
                        {{ setting_item('map_contact_long') }}
                    ],
                    zoom: {{ setting_item('map_contact_zoom') ?? '8' }},
                    ready: function(engineMap) {
                        engineMap.addMarker([{{ setting_item('map_contact_lat') }},
                            {{ setting_item('map_contact_long') }}
                        ], {
                            icon_options: {}
                        });

...
jQuery(function ($) {
    ...
    $(".bc_user_profile .sidebar-menu .caret").on('click',function () {
        $(this).closest("li").toggleClass("active_child");
    });

    $(".bc_user_profile .bc-list-item .control-action .btn-danger").on('click',function () {
    ...
    $(".bc_user_profile .bc-list-item .control-action .btn-recovery").on('click',function () {
        var c = confirm($(this).data('confirm'));
        if(!c){
            return false;
        }
    ...
    $('.form-add-service .tab-content .tab-pane a').on('click',function () {
        setTimeout(function () {
            window.dispatchEvent(new Event('resize'));
        },200)
    });
